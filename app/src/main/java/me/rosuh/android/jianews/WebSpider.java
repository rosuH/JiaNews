    ...
    private static List<Article> dataFilterForList(Elements links, Elements dates, int index) {
        ...
        try {
            for (int i = cirStart; i < index; i++) {
                ...
                article.setUrl(link.attr("abs:href"));
                article.setTitle(link.text());
                Element contentBody = Jsoup
                        .connect(article.getUrl()).get().body();
                ...
                article.setContent(realContent.toString());
                article.setSummary(realContent.text());

//                if (contentTableInsideTr.getElementsByTag("div").size() > 1){
////                    article.setContent(contentTableInsideTr.getElementsByTag("div").toString());
////                }else {
////                    article.setContent(contentTableInsideTr.getElementsByTag("p").toString());
////                }
                /**
                 * 获取第一张图片链接作为缩略图
                 * 如果为空，则不获取，防止抛出异常
                ...
                if (!imgLinks.isEmpty()) {
                    article.setThumbnail(imgLinks.get(0).attr("abs:src"));
                }
